...
下面是新包的结构示例，名称、描述和视觉参数须按新方向填写。两个现有清单也可作为字段参考，但不要继承不属于新方向的材质、配色或提示词禁令。
...
4. 运行共享 `scripts/selftest.py`。它会验证动态第三风格在两种类型中可构建、风格资源独立及缺失／重名／草稿等错误处理；实际新主题的视觉仍需第 3 步检查。
