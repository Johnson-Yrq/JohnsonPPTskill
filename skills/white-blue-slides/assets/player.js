(()=>{'use strict';
 ...
 $('#fullscreen').onclick=async()=>{try{if(document.fullscreenElement)await document.exitFullscreen();else{setEdit(false);setOverview(false);$('#notes-panel').hidden=true;await document.body.requestFullscreen();document.activeElement?.blur();fit();}}catch{toast('请使用浏览器的全屏功能')}};
 $('#notes').onclick=()=>{$('#notes-panel').hidden=!$('#notes-panel').hidden};$('#close-notes').onclick=()=>{$('#notes-panel').hidden=true};
 $('#print').onclick=async()=>{
  if(document.querySelector('.chart-shell[data-invalid]')){toast('请先修正图表数据：类别不能为空，数值须为非负数');return}
  document.activeElement?.blur();await document.fonts.ready;
  await Promise.all([...document.images].map(im=>im.decode().catch(()=>{})));
  window.print();
 };
 $('#save').onclick=()=>{if(document.querySelector('.chart-shell[data-invalid]')){toast('请先修正图表数据：类别不能为空，数值须为非负数');return}setEdit(false);const clone=document.documentElement.cloneNode(true);clone.querySelector('#notes-panel').hidden=true;clone.querySelector('#status').classList.remove('visible');clone.querySelector('body').classList.remove('overview-mode');const data='<!DOCTYPE html>\n'+clone.outerHTML,url=URL.createObjectURL(new Blob([data],{type:'text/html;charset=utf-8'})),a=document.createElement('a');a.href=url;a.download=(document.title.replace(/[\\/:*?"<>|]/g,'-')||'演示稿')+'-已编辑.html';a.click();setTimeout(()=>URL.revokeObjectURL(url),2000);toast('已保存独立 HTML，文字与图片均已包含')};
 $('#pptx').onclick=async()=>{if(document.querySelector('.chart-shell[data-invalid]')){toast('请先修正图表数据：类别不能为空，数值须为非负数');return}if(!window.deckPptx){toast('此演示稿不含 PPTX 导出器，请用 scripts/export_pptx.cjs 导出');return}setEdit(false);setOverview(false);$('#pptx').disabled=true;toast('正在生成 PPTX…');try{const r=await window.deckPptx.download();toast(`已导出 PPTX：${r.pages} 页，统一使用 ${r.font}，文字与图表可编辑`)}catch(e){toast('导出失败：'+e.message)}finally{$('#pptx').disabled=false;show(current)}};
